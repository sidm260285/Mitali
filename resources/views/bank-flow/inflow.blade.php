...
@section('title', 'Bank Inflow - Mitali SP')

@section('content')
    @php
        $isAdmin = auth()->user()->isAdmin();
        $balanceBaseUrl = $isAdmin
            ? rtrim(route('admin.bank-flow.inflow.create'), '/inflow').'banks'
            : rtrim(route('executive.bank-flow.inflow.create'), '/inflow').'banks';
    @endphp

    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0">Bank Inflow</h4>
    </div>

    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form method="POST" action="{{ $isAdmin ? route('admin.bank-flow.inflow.store') : route('executive.bank-flow.inflow.store') }}">
                @csrf
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="bank_id" id="bank_id"
                                ...
                                data-balance-base="{{ $isAdmin ? url('admin/bank-flow/banks') : url('executive/bank-flow/banks') }}"
                                required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="account_head_id" id="account_head_id"
                                class="form-select @error('account_head_id') is-invalid @enderror"
                                required>
                ...
                <div class="form-actions mt-4">
                    <button type="submit" class="btn btn-form btn-form-primary">
                        <i class="bi bi-plus-circle"></i> Record Bank Inflow
                    </button>
                </div>
            </form>
        </div>

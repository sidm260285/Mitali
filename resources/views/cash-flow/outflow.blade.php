...
@section('title', 'Outflow - Mitali SP')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0">Outflow</h4>
    </div>

    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form method="POST" action="{{ auth()->user()->isAdmin() ? route('admin.cash-flow.outflow.store') : route('executive.cash-flow.outflow.store') }}">
                @csrf
                ...
                <div class="form-actions mt-4">
                    <button type="submit" class="btn btn-form btn-form-primary">
                        <i class="bi bi-dash-circle"></i> Record Outflow
                    </button>
                </div>
            </form>
        </div>
